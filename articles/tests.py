...
class ArticleTestCase(TestCase):
    def setUp(self):
        ...
        for i in range(0,self.number_of_articles):
            Article.objects.create(title='tomasito', content='aaa')
    ...
    def test_tomasito_unique_slug(self):
        qs = Article.objects.exclude(slug__iexact='tomasito')
        for obj in qs: 
            title = obj.title
            slug = obj.slug
            slugified_title = slugify(title)
    ...
    def test_article_search_manager(self):
        qs = Article.objects.search(query="tomasito")
        self.assertEqual(qs.count(), self.number_of_articles)
        qs = Article.objects.search(query="")
        self.assertEqual(qs.count(), 0)
        qs = Article.objects.search(query="a")
        self.assertEqual(qs.count(), self.number_of_articles)
